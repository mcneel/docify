          ...
          <div v-for="(member, index) in members.items" :key="index">
                ...
                <q-item-label caption class="on-right row" v-for="parameter in member.parameters" :key="parameter.name">
                  <ul
                    ...
                    <li v-if="paramTypes[parameter.name] && paramTypes[parameter.name].link">
                      <router-link class="routerlink" :to="paramTypes[parameter.name].link">
                        {{ paramTypes[parameter.name].path }}
                      </router-link>
                    </li>
                    <li v-if="paramTypes[parameter.name] && !paramTypes[parameter.name].link" class="disabled disabledLink">
                      {{
                        paramTypes[parameter.name].type }}</li>
                    <li v-html="parameter.summary"></li>
                  </ul>

                </q-item-label>
                <q-item-label caption v-if="member.returns" class="text-bold"
                  style="margin-top: 10px;">Returns:</q-item-label>
                <q-item-label caption class="on-right" v-if="member.returns">
                  {{ member.returns }}
                </q-item-label>
                <q-item-label caption v-if="member.since" class="text-bold" style="margin-top: 10px;">Available
                  since:</q-item-label>
